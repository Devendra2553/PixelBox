  ...
  return (
    <div className="p-4 sm:p-6 min-h-screen bg-gray-100">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        <div>
          <h2 className="text-2xl font-bold text-gray-800">Manage All Artworks</h2>
          <p className="text-sm text-gray-500">Total: {artworks.length} items</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        {filteredArt.length > 0 ? (
          filteredArt.map((item) => (
            <div key={item._id} className="bg-white rounded-2xl shadow-sm overflow-hidden flex flex-col hover:shadow-md transition group border border-gray-200">
              ...
              <div className="p-4 flex flex-col grow">
                ...
                <p className="text-xs text-gray-500">By <span className="font-medium text-gray-700">{item.artistName || "Unknown"}</span></p>
                <p className="text-[11px] text-gray-400 uppercase mt-2 italic">{item.category}</p>
              </div>
            </div>
          ))
        ) : (
